...
// Ruixen Gradient Footer — a normal footer that sits at the bottom of the
// page. Its content reads first; the blurred rainbow is pinned to the bottom
// of the viewport and stretches up from the floor over the last stretch of
// scroll, hitting full height exactly when you reach the end of the page.
// One inline <svg> — no canvas, no giant scroll spacer.
//
// Ported from the original TSX registry component: types stripped (this
// project is plain JS/JSX, no TypeScript), Tailwind-free by design already —
// the component itself never used Tailwind, only its demo markup did.
//
...
/**
 * @param {object} props
 * @param {import('react').ReactNode} [props.children] Footer content — links, wordmark, copyright — shown above the glow.
 * @param {string} [props.gradientHeight] Height of the glow band pinned to the viewport bottom. Doubles as the scroll distance the reveal takes, and the room reserved under the content.
 * @param {number} [props.minReveal] Resting height of the glow, as a fraction of the band (0..1).
 * @param {number} [props.bars] Number of blurred columns.
 * @param {number} [props.blur] Blur in viewBox units.
 * @param {number} [props.peak] Peak height as a fraction of the viewBox.
 * @param {number} [props.valley] Edge height as a fraction of the peak (0..1).
 * @param {{offset:number,color:string}[]} [props.stops] Vertical rainbow gradient stops, floor (0) → top (1).
...
export function RuixenGradientFooter({
  ...
  useEffect(() => {
    ...
    // on a real page and inside an iframe preview alike.
  ...
  return (
    ...
    <footer className={className} style={{ paddingBottom: gradientHeight, ...style }}>
      ...
      {/* pinned to the viewport — a transformed/filtered ancestor would
          capture it. Give the footer a plain containing block. */}
      <div
        ref={bandRef}
        aria-hidden="true"
        style={{
